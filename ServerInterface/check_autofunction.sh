...
# Header
log "Prüfe Status des AutoFunction Servers..." "${BLUE}"
echo "----------------------------------------"
...
if pgrep -f "autoFunction_server.py" > /dev/null; then
    PID=$(pgrep -f "autoFunction_server.py")
    log "✓ AutoFunction Server Prozess gefunden (PID: $PID)" "${GREEN}"
    # Zeige Prozess-Details
    log "Prozess-Details:" "${BLUE}"
    ps -p $PID -o pid,ppid,user,%cpu,%mem,start,time,command
else
    log "✗ Kein AutoFunction Server Prozess gefunden!" "${RED}"
fi
echo "----------------------------------------"

# 3. Port-Check
log "Port-Check (8128):" "${YELLOW}"
if command -v netstat > /dev/null; then
    if netstat -tuln | grep ":8128 " > /dev/null; then
        log "✓ Port 8128 ist aktiv" "${GREEN}"
        # Zeige detaillierte Port-Informationen
        netstat -tuln | grep ":8128 "
    else
        log "✗ Port 8128 ist nicht aktiv!" "${RED}"
    fi
else
...
if systemctl is-active --quiet $SERVICE_NAME && pgrep -f "autoFunction_server.py" > /dev/null && netstat -tuln | grep ":8128 " > /dev/null; then
    log "✓ AutoFunction Server läuft normal" "${GREEN}"
else
    log "✗ AutoFunction Server hat Probleme" "${RED}"
